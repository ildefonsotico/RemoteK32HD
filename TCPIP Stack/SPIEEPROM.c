...
#define EEPROM_MAX_SPI_FREQ		(10000000ul)	// Hz
	...
	#define WaitForDataByte()	{while ((EEPROM_SPISTATbits.SPITBF == 1) || (EEPROM_SPISTATbits.SPIRBF == 0));}
	#define SPI_ON_BIT			(EEPROM_SPISTATbits.SPIEN)
#elif defined( __PIC32MX__ )
	#define WaitForDataByte()	{while (!EEPROM_SPISTATbits.SPITBE || !EEPROM_SPISTATbits.SPIRBF);}
