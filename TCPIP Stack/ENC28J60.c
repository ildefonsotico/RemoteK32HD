...
/*
unsigned char f,j;       //Variáveis auxiliares
LEDEn=0;          // Acende LED Enable
LEDAu=0;          // Acende LED de Audio
...
LEDVi=1;          // Apaga LED de Video
LEDEn=1;          // Apaga LED Enable
*/
...
BYTE MACGet()
{
   ...
   #if defined(__C32__)
   {
      ...
      while(!ENC_SPI_IF);         // Wait until WORD is transmitted
   ...
   #if defined(__C32__)
   {
      ...
      if(len >= 4)
      {
         ...
         {         
            ...
            i += 4;
            while(!ENC_SPI_IF);         // Wait until DWORD is transmitted
            ENC_SPI_IF = 0;
            dwv.Val = ENC_SSPBUF;
            if(val)
            {
...
void MACPut(BYTE val)
{
   ...
   #else
   {
      ...
      while(!ENC_SPI_IF);      // Wait until finished transmitting
   }
   #endif
